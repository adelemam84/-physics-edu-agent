...
def whatsapp_status():
    return {
        ...
        "webhook_verify_token_configured": bool(os.getenv("WHATSAPP_WEBHOOK_VERIFY_TOKEN","").strip()),
        "meta_app_secret_configured": bool(os.getenv("META_APP_SECRET","").strip()),
        "webhook_ready": bool(os.getenv("WHATSAPP_WEBHOOK_VERIFY_TOKEN","").strip() and os.getenv("META_APP_SECRET","").strip()),
...
@app.get("/api/guardians/import-template.csv", dependencies=[Depends(require_admin)])
def guardians_import_template():
    body="student_code,guardian_name,whatsapp_phone,relationship,whatsapp_opt_in\nSCI-XXXXXXXXXX,Guardian Example,,Parent,yes\n"
    return Response("\ufeff"+body,media_type="text/csv; charset=utf-8",
                    headers={"Content-Disposition":"attachment; filename=guardians_import_template.csv"})
...
def list_notifications(limit: int = 200):
    with connect() as con:
        return list(con.execute(
            """SELECT n.id,n.notification_type,n.status,n.delivery_status,n.attempts_count,n.created_at,n.sent_at,n.delivered_at,n.read_at,n.provider_status_at,n.provider_error_code,n.provider_error_title,n.error_message,n.payload,
...
<h1>أولياء الأمور وإشعارات واتساب</h1><div class="box row"><a href="/admin/dashboard">جلسة الإدارة</a><a href="/admin">لوحة الإدارة</a><span id=msg></span></div>
<div class=box><h3>حالة WhatsApp Cloud API</h3><div id=wa></div><div id=webhookHelp class=muted style="margin-top:10px"></div></div>
...
<div class=box><h3>استيراد أولياء الأمور دفعة واحدة</h3><p class=muted>CSV بالأعمدة: student_code, guardian_name, whatsapp_phone, relationship, whatsapp_opt_in.</p><div class=row><input id=gcsv type=file accept=".csv,text/csv"><button onclick=importGuardians()>استيراد CSV</button><button onclick="downloadGuardianTemplate()">تحميل نموذج CSV</button></div></div>
...
<script>
function h(){return {}}function saveKey(){location.href='/admin/login'}
