...
<div class=row><select id=subject><option value="">المادة</option></select><select id=grade><option value="">الصف</option></select></div>
...
function pick(id){selected=allDocs.find(d=>d.id==id);title.textContent='#'+selected.id+' '+selected.filename;openBtn.disabled=false;previewBtn.disabled=false;assignBtn.disabled=false;extractBtn.disabled=true;img.removeAttribute('src');msg.textContent='راجع الملف ثم اختر السياق الأكاديمي الصحيح.';load()}
...
function filterTerms(){let cid=+curriculum.value||0;let rows=(catalog?.terms||[]).filter(x=>!cid||x.curriculum_version_id==cid);opt(term,rows,'name_ar','الترم')}
subject.onchange=filterCurricula;grade.onchange=filterCurricula;curriculum.onchange=filterTerms;
...
async function assign(){if(!selected)return;if(!subject.value||!grade.value||!curriculum.value||!term.value){msg.className='bad';msg.textContent='اختر المادة والصف وإصدار المنهج والترم';return}msg.className='muted';msg.textContent='جارٍ حفظ التصنيف...';let x=await jf('/api/documents/'+selected.id+'/academic-context',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({subject_id:+subject.value,grade_level_id:+grade.value,curriculum_version_id:+curriculum.value,term_id:+term.value})});msg.className='ok';msg.textContent='تم حفظ التصنيف. يمكنك الآن إعادة استخراج الأسئلة من الصفحات المفهرسة.';extractBtn.disabled=false}
