...
app=FastAPI(title="Physics Educational AI Agent",version="0.8.1",lifespan=lifespan)
...
async def upload_document(file:UploadFile=File(...),subject:str=Form('physics'),kind:str=Form('questions')):
    ...
    digest=hashlib.sha256(raw).hexdigest(); doc_id=None
    with NamedTemporaryFile(suffix='.pdf',delete=False) as tmp: tmp.write(raw); path=Path(tmp.name)
    try:
        with connect() as con:
            doc_id=con.execute('INSERT INTO documents(filename,subject,kind,status) VALUES (%s,%s,%s,%s) RETURNING id',(file.filename,subject,kind,'processing')).fetchone()['id']
        pdf_key=f'documents/{doc_id}/{digest}.pdf'; put_bytes(pdf_key,raw,'application/pdf'); added=0
        page_rows=extract_pages(path)
        with connect() as con:
            con.execute('INSERT INTO document_files(document_id,bucket_name,object_key,content_type,file_sha256,file_size_bytes,page_count) VALUES (%s,%s,%s,%s,%s,%s,%s)',(doc_id,BUCKET,pdf_key,'application/pdf',digest,len(raw),page_count))
...
ADMIN='''<!doctype html><html lang="ar" dir="rtl"><meta name="viewport" content="width=device-width,initial-scale=1"><title>إدارة بنك الأسئلة</title><style>body{font-family:system-ui;background:#f5f7fb;margin:0;color:#172033}main{max-width:1200px;margin:auto;padding:20px}.box{background:#fff;border-radius:16px;padding:16px;margin:12px 0}.grid{display:grid;grid-template-columns:360px 1fr;gap:14px}input,select,button{padding:10px;border:1px solid #ccd2dd;border-radius:9px}button{cursor:pointer}.q{padding:10px;border-bottom:1px solid #eee;cursor:pointer}.q:hover{background:#f7f9fc}pre{white-space:pre-wrap;font-family:inherit}@media(max-width:760px){.grid{grid-template-columns:1fr}}</style><main><h1>لوحة إدارة الفيزياء</h1><div class=box><input id=key type=password placeholder="ADMIN_API_KEY"><button onclick="save()">حفظ المفتاح على هذا الجهاز</button><input id=pdf type=file accept="application/pdf"><button onclick="upload()">رفع PDF أسئلة</button><span id=msg></span></div><div class=grid><div class=box><h3>الأسئلة</h3><div id=list></div></div><div class=box id=review>اختر سؤالًا للمراجعة</div></div></main><script>let qs=[],sel=null;key.value=localStorage.pk||'';function save(){localStorage.pk=key.value;load()}function h(){return {'X-Admin-Key':localStorage.pk||''}}async function load(){qs=await fetch('/api/questions?limit=500').then(r=>r.json());list.innerHTML=qs.map(q=>`<div class=q onclick="pick(${q.id})">#${q.id} · ${q.source_filename} · صفحة ${q.page}<br>${q.text_verbatim.slice(0,140)}</div>`).join('')}function pick(id){sel=qs.find(x=>x.id==id);review.innerHTML=`<h3>${sel.source_filename} — صفحة ${sel.page}</h3><img id=img style="max-width:100%;border-radius:10px"><pre>${sel.text_verbatim}</pre><input id=ans placeholder="الإجابة المعتمدة" value="${sel.accepted_answer||''}"><button onclick="patch(true)">اعتماد</button><button onclick="patch(false)">إلغاء الاعتماد</button><button onclick="openPdf()">فتح PDF</button>`;fetch(`/api/documents/${sel.document_id}/page/${sel.page}/preview`,{headers:h()}).then(r=>r.blob()).then(b=>img.src=URL.createObjectURL(b))}async function patch(v){await fetch('/api/questions/'+sel.id,{method:'PATCH',headers:{...h(),'Content-Type':'application/json'},body:JSON.stringify({approved:v,accepted_answer:ans.value||null})});load()}async function openPdf(){let x=await fetch(`/api/documents/${sel.document_id}/pdf-url`,{headers:h()}).then(r=>r.json());window.open(x.url,'_blank')}async function upload(){let f=pdf.files[0];if(!f)return;let d=new FormData();d.append('file',f);d.append('kind','questions');let r=await fetch('/api/documents/upload',{method:'POST',headers:h(),body:d});let x=await r.json();msg.textContent=r.ok?`تم: ${x.candidate_questions_added} سؤال مرشح`:(x.detail||'خطأ');if(r.ok)load()}load()</script></html>'''
