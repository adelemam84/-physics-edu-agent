...
def normalize_design_spec(value: Any) -> tuple[dict, list[str]]:
    blockers: list[str] = []
    if value in (None, {}):
        return {"layout": "standard", "theme": copy.deepcopy(_DEFAULT_THEME), "elements": copy.deepcopy(_DEFAULT_RECTS)}, blockers
    if not isinstance(value, dict):
        return {}, ["design_spec_invalid"]
    if set(value) - {"layout", "theme", "elements"}:
    ...
    return {"layout": layout, "theme": theme, "elements": elements}, list(dict.fromkeys(blockers))
...
def validate_presentation_advanced_design_edits(
    ...
    *,
    ...
    base_by_id = {str(x.get("slide_id") or ""): x for x in (base_blueprint.get("slides") or []) if isinstance(x, dict)}
    for slide in edited_blueprint.get("slides") or []:
        if not isinstance(slide, dict):
            continue
        sid = str(slide.get("slide_id") or "")
    ...
    if isinstance(prepared, dict):
        ...
        if design_changed:
            ...
            prepared["approval_state"] = {"teacher_approved": False, "revision": base_revision + 1, "edited": True}
    ...
    report["edit_digest"] = base_editor.presentation_edit_digest(
        base_editor.presentation_editor_base_hash(base_blueprint), edited_blueprint
    )
...
def _align(value: str) -> PP_ALIGN:
    return {"left": PP_ALIGN.LEFT, "center": PP_ALIGN.CENTER, "right": PP_ALIGN.RIGHT}.get(value, PP_ALIGN.RIGHT)
...
def _inch_rect(spec: dict, role: str) -> tuple:
    ...
    return Inches(13.333333 * x / 100), Inches(7.5 * y / 100), Inches(13.333333 * w / 100), Inches(7.5 * h / 100)


def _text_box(slide, rect: tuple, text: str, *, size: float, color: str, font: str, align: str, bold: bool = False):
...
def _render_visual(slide, visual: dict, rect: tuple, theme: dict) -> None:
    ...
    for key in ("nodes", "steps", "items", "labels"):
        ...
        if isinstance(values, list):
            for item in values:
                text = (item.get("label") or item.get("title") or item.get("text")) if isinstance(item, dict) else item
    ...
    for i, label in enumerate(labels):
        ...
        shape.fill.solid(); shape.fill.fore_color.rgb = _rgb("#FFFFFF")
        shape.line.color.rgb = _rgb(theme["accent"])
        frame = shape.text_frame; frame.clear(); p = frame.paragraphs[0]
        p.text = label; p.alignment = PP_ALIGN.CENTER; p.font.size = Pt(13 * float(theme["content_scale"])); p.font.name = theme["font_family"]; p.font.color.rgb = _rgb(theme["text_color"])
    ...
    for r, row in enumerate(table.rows):
        for cell in row.cells:
            ...
            for p in cell.text_frame.paragraphs:
                p.alignment = _align(theme["body_align"]); p.font.size = Pt(11 * float(theme["content_scale"])); p.font.name = theme["font_family"]
...
def render_presentation_pptx_with_design(blueprint: dict, edition: str) -> bytes:
    has_design = any(isinstance(x, dict) and x.get("design_spec") for x in (blueprint.get("slides") or []))
    if not has_design:
    ...
    for spec in projected.get("slides") or []:
        ...
        bg = slide.background.fill; bg.solid(); bg.fore_color.rgb = _rgb(theme["background"])
        scale = float(theme["content_scale"]); font = theme["font_family"]
        _text_box(slide, _inch_rect(design, "title"), spec.get("title") or "", size=28 * scale, color=theme["title_color"], font=font, align=theme["title_align"], bold=True)
        blocks = [x for x in (spec.get("content_blocks") or []) if isinstance(x, dict)]
        if blocks:
            rect = _inch_rect(design, "body")
            body = slide.shapes.add_textbox(*rect); frame = body.text_frame; frame.clear()
            for i, block in enumerate(blocks):
                p = frame.paragraphs[0] if i == 0 else frame.add_paragraph(); prefix = "• "
                if block.get("kind") == "answer": prefix = "الإجابة: "
                elif block.get("kind") == "explanation": prefix = "التفسير: "
                p.text = prefix + str(block.get("text") or ""); p.alignment = _align(theme["body_align"]); p.font.size = Pt((17 if len(blocks) <= 5 else 14) * scale); p.font.name = font; p.font.color.rgb = _rgb(theme["text_color"])
        ...
        if visuals: _render_visual(slide, visuals[0], visual_rect, theme)
        elif tables: _render_table(slide, tables[0], visual_rect, theme)
        refs = " · ".join(spec.get("source_refs") or [])
        if refs:
            _text_box(slide, (Inches(.55), Inches(7.02), Inches(12.2), Inches(.26)), refs, size=7.5, color=theme["text_color"], font=font, align="right")
        if edition == "teacher": _add_notes(slide, list(spec.get("speaker_notes") or []))
