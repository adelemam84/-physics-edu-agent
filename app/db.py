...
def init_db():
    ...
    with connect() as con:
        ...
        con.execute("CREATE INDEX IF NOT EXISTS idx_parent_notifications_student ON parent_notifications(student_id)")
        con.execute("CREATE INDEX IF NOT EXISTS idx_questions_answer_document ON questions(answer_document_id) WHERE answer_document_id IS NOT NULL")
        con.execute("CREATE INDEX IF NOT EXISTS idx_quizzes_lesson ON quizzes(lesson_id) WHERE lesson_id IS NOT NULL")
