...
def integration_control_snapshot() -> dict:
    ...
    return {
        ...
        'providers': status,
        'actions': {
            'canva': '/api/admin/lesson-studio/jobs/{job_id}/external/canva',
            'google_slides': '/api/admin/lesson-studio/jobs/{job_id}/external/google-slides',
            'gemini_notebook_enterprise': '/api/admin/lesson-studio/jobs/{job_id}/external/gemini-notebook',
        },
        'policy': {
...
<div class=card><h1>مركز التكاملات الإبداعية</h1><p class=muted>تشغيل Canva وGoogle Slides وGemini Notebook من المحتوى المعتمد في Lesson Studio. التكاملات لا تغيّر المحتوى العلمي ولا تتجاوز بوابات المراجعة.</p><div class=row><input id=jid placeholder="رقم مشروع الدرس"><button id=refresh>تحديث الحالة</button></div></div>
...
async function load(){try{state=await api('/api/admin/lesson-studio/integrations/summary');render()}catch(e){$('#canvaResult').textContent=e.message}}
...
async function runExternal(kind,out){if(!jid()){alert('أدخل رقم مشروع الدرس أولًا');return}let map={canva:'canva',slides:'google-slides',notebook:'gemini-notebook'},key=map[kind];out.textContent='جارٍ التنفيذ...';try{let x=await api(`/api/admin/lesson-studio/jobs/${encodeURIComponent(jid())}/external/${key}`,{method:'POST'}),u=resultLink(x);out.innerHTML=(u?`<a href="${esc(u)}" target=_blank rel="noopener">فتح النتيجة</a><br>`:'')+`<span class=muted>${esc(JSON.stringify(x,null,2))}</span>`}catch(e){out.textContent=e.message}}
$('#refresh').onclick=load;$('#authorize').onclick=()=>{location.href='/api/admin/integrations/canva/oauth/start'};$('#remote').onclick=async()=>{let o=$('#canvaResult');o.textContent='جارٍ فحص Dataset...';try{let x=await api('/api/admin/integrations/canva/diagnostics?remote=true');o.textContent=JSON.stringify(x,null,2);await load()}catch(e){o.textContent=e.message}};$('#createCanva').onclick=()=>runExternal('canva',$('#canvaResult'));$('#createSlides').onclick=()=>runExternal('slides',$('#slidesResult'));$('#createNotebook').onclick=()=>runExternal('notebook',$('#notebookResult'));let q=new URLSearchParams(location.search).get('job_id');if(q)$('#jid').value=q;load();
</script></main></html>'''
