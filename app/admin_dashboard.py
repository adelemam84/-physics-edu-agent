...
async function load(){let s=await fetch('/api/admin/session').then(r=>r.json());if(!s.authenticated){location.href='/admin/login';return}msg.textContent='جلسة الإدارة آمنة ومفعلة';let r=await fetch('/api/admin/dashboard',{headers:H()});if(!r.ok){location.href='/admin/login';return}let x=await r.json(),s=x.summary||{};let ar=await fetch('/api/admin/alert-center').then(r=>r.json()).catch(()=>null);alertSummary.innerHTML=ar?(ar.alerts.length?'<b class="'+(ar.error_count?'bad':'warn')+'">⚠️ '+(ar.error_count+ar.warning_count)+' تنبيه يحتاج مراجعة</b> · <a href="/admin/alerts">فتح مركز التنبيهات</a>':'<span class=ok>✅ لا توجد تنبيهات تشغيلية حالية</span>'):'<span class=muted>تعذر تحميل مركز التنبيهات</span>';cards.innerHTML=[['الطلاب',s.students||0],['أولياء الأمور',s.guardians||0],['الاختبارات',s.quizzes||0],['الاختبارات المنشورة',s.published_quizzes||0],['المحاولات',s.attempts||0],['الأسئلة المعتمدة',s.approved_questions||0],['متوسط النتائج',(s.avg_score??0)+'%'],['رسائل في الانتظار',s.queued_notifications||0],['رسائل فاشلة',s.failed_notifications||0]].map(([a,b])=>`<div class=card><div class=muted>${a}</div><h2>${b}</h2></div>`).join('');
