...
def student_portal(student_code:str):
    ...
    with connect() as con:
        ...
        attempts=list(con.execute("""SELECT a.id,a.quiz_id,q.title quiz_title,a.score,a.max_score,a.completed_at,
          ...
          WHERE a.student_id=%s ORDER BY a.completed_at DESC NULLS LAST,a.id DESC LIMIT 10""",(st["id"],st["id"],st["id"],st["id"],st["id"])).fetchall())
        ...
        available=list(con.execute("""SELECT q.id,q.title,q.duration_minutes,q.quality_score,q.published_at,q.max_attempts,q.retry_wait_minutes,q.score_policy,
          ...
          ORDER BY q.published_at DESC NULLS LAST,q.id DESC LIMIT 50""",(st["id"],)).fetchall())
    from .analytics import _student_mastery
    with connect() as con:
    ...
    return {"student":st,"attempts":attempts,"errors":errors,"available_quizzes":available,"mastery":mastery,"adaptive":adaptive,
