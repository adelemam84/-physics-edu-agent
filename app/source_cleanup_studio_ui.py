...
<div class=actions style="margin-top:12px"><button id=analyze>Analyze</button><button id=enhance class=primary>Auto Enhance</button><button id=approve>اعتماد التحسين</button><button id=verifyOcr>OCR Before/After</button><button id=remove>حذف النسخة المحسنة</button><button id=batch>Batch للصفحات منخفضة الجودة</button><button id=continueOcr class=primary>متابعة OCR وإنشاء الملزمة</button></div><div id=state class=muted style="margin-top:10px"></div></div>
...
$('continueOcr').onclick=async()=>{if(!confirm('متابعة OCR؟ الصفحات المعتمدة ستستخدم نسخة OCR المحسنة، وباقي الصفحات ستستخدم الأصل.'))return;$('globalState').textContent='جارٍ OCR صفحة بصفحة...';try{let done=false;while(!done){let r=await fetch('/api/admin/lesson-pack-studio/jobs/'+id+'/process-next',{method:'POST'});let x=await r.json();if(!r.ok)throw new Error(JSON.stringify(x.detail||x));done=!!x.done;$('globalState').textContent='OCR processed: '+x.processed+' · Review pages: '+x.review_pages}let fd=new FormData();fd.append('question_count','12');let g=await fetch('/api/admin/lesson-pack-studio/jobs/'+id+'/generate',{method:'POST',body:fd});let y=await g.json();if(!g.ok)throw new Error(JSON.stringify(y.detail||y));location.href='/admin/lesson-pack-studio/jobs/'+id+'/preview'}catch(e){$('globalState').className='bad';$('globalState').textContent='خطأ: '+e.message}};
load();
</script></main></html>'''
