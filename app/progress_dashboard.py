...
<div class=box><a href="/admin/dashboard">لوحة التحكم</a> <a href="/admin/dashboard">لوحة التحكم</a></div><div id=content></div>
<script>function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}async function load(){let r=await fetch('/api/admin/progress',{});if(!r.ok){content.innerHTML='<div class=box>تعذر فتح التقرير</div>';return}let x=await r.json();content.innerHTML=`<div class=box><h1>متابعة تقدم الطلاب</h1><div class=grid><div class=card><div class=muted>الطلاب</div><div class=big>${x.summary.students||0}</div></div><div class=card><div class=muted>المحاولات</div><div class=big>${x.summary.attempts||0}</div></div><div class=card><div class=muted>متوسط النتائج</div><div class=big>${x.summary.avg_score||0}%</div></div></div></div><div class=box><h2>حسب المادة</h2><div class=grid>${x.subjects.map(s=>'<div class=card><b>'+esc(s.name_ar)+'</b><div class=big>'+esc(s.avg_score||0)+'%</div><div class=muted>'+s.attempts+' محاولة</div></div>').join('')}</div></div><div class=box><h2>أولوية التدخل</h2><table><tr><th>الطالب</th><th>المادة</th><th>الدرس</th><th>الإتقان</th><th></th></tr>${x.weakest.map(w=>'<tr><td>'+esc(w.student_name)+'</td><td>'+esc(w.subject_name||'')+'</td><td>'+esc(w.lesson_title)+'</td><td class=bad>'+esc(w.mastery)+'%</td><td><a href="/admin/students/'+w.student_id+'/knowledge-map">الخريطة</a></td></tr>').join('')}</table></div>`}</script></main></html>'''
