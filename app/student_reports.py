...
def attempt_report(attempt_id:int):
    with connect() as con:
        ...
        overrides=list(con.execute("""SELECT previous_score,new_score,reason,actor,created_at
          FROM attempt_score_overrides WHERE attempt_id=%s ORDER BY created_at DESC,id DESC""",(attempt_id,)).fetchall())
        pct=round(float(a["score"] or 0)/float(a["max_score"] or 1)*100,1) if a["max_score"] else 0
        return {"attempt":{**a,"percentage":pct},"answers":answers,"lessons":lessons,"difficulty":difficulty,"history":history,"score_overrides":overrides}
...
<div class=box><a href="/admin/dashboard">لوحة التحكم</a> · <a id=diag href="#">التحليل التشخيصي</a></div><div id=head></div><div id=lesson></div><div id=diff></div><div id=wrong></div><div id=history></div>
<script>let id=Number(location.pathname.split('/').pop());const H=()=>({});function save(){load()}function e(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}
...
diag.href='/api/admin/attempts/'+id+'/diagnostic';
