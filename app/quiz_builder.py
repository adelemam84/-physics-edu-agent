...
def quiz_quality_check(quiz_id:int):
    with connect() as con:
        ...
        rows=list(con.execute("""SELECT q.id,q.lesson_id,q.difficulty,q.question_type,
        ...
        expected=con.execute("""SELECT count(*) n FROM lessons l WHERE l.subject_id=%s AND l.grade_level_id=%s
          AND l.curriculum_version_id=%s AND l.term_id=%s""",
          (quiz['subject_id'],quiz['grade_level_id'],quiz['curriculum_version_id'],quiz['term_id'])).fetchone()
    n=len(rows)
    ...
    target=min(n,max(1,int(expected['n'] or 0)));lesson_cov=len(lessons)/target
    concept_cov=min(1,len(concepts)/max(1,target));skill_cov=min(1,len(skills)/max(1,min(n,3)))
    ...
    checks=[
      {'id':'lesson_coverage','label':'تغطية الدروس','ok':lesson_cov>=.8,'value':round(lesson_cov*100)},
      {'id':'concept_coverage','label':'تنوع المفاهيم','ok':concept_cov>=.8,'value':len(concepts)},
      {'id':'skill_coverage','label':'تنوع المهارات','ok':skill_cov>=.67,'value':len(skills)},
      {'id':'difficulty_mix','label':'تنوع مستويات الصعوبة','ok':len(diffs)>=min(3,n),'value':diffs},
      {'id':'question_type_mix','label':'تنوع أنواع الأسئلة','ok':len(types)>=min(2,n),'value':types},
