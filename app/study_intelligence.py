...
def _lesson_source(con, lesson_id: int | None):
    if not lesson_id:
        return None
    return con.execute(
        """SELECT lsm.document_id,lsm.start_page,lsm.end_page,d.filename,d.kind
...
body{font-family:system-ui;background:#f5f7fb;color:#172033;margin:0}main{max-width:1000px;margin:auto;padding:16px}.box{background:#fff;border:1px solid #e4e7ec;border-radius:16px;padding:16px;margin:12px 0}.task{border:1px solid #e4e7ec;border-radius:13px;padding:13px;margin:9px 0}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px}.card{border:1px solid #e4e7ec;border-radius:12px;padding:12px}.big{font-size:25px;font-weight:800}.bad{color:#b42318}.warn{color:#b54708}.ok{color:#067647}.muted{color:#667085}button{padding:9px 12px;border:1px solid #ccd2dd;border-radius:9px;background:#2447a8;color:white;cursor:pointer}button:disabled{opacity:.6;cursor:wait}a{color:#175cd3;text-decoration:none}button:focus-visible,a:focus-visible{outline:3px solid #84adff;outline-offset:2px}@media(max-width:600px){main{padding:10px}.box{padding:13px;border-radius:13px}.grid{grid-template-columns:1fr 1fr}}@media(max-width:390px){.grid{grid-template-columns:1fr}}</style><main>
<div class=box><a href="/student">← بوابة الطالب</a> · <a href="/student/command-center">مهمة اليوم</a><h1>قائمة المذاكرة الشخصية</h1><p class=muted>الأولوية مبنية على آخر 5 محاولات مقارنة بالـ5 السابقة، وليس على تقدير AI.</p><div id=taskMsg class=muted role=status aria-live=polite></div></div><div id=content class=box role=status aria-live=polite>جارٍ تحميل الخطة...</div>
<script>const content=document.getElementById('content'),taskMsg=document.getElementById('taskMsg');const statusLabels={worsening:'يتراجع',persistent_weakness:'ضعف مستمر',developing:'قيد التطور',improving:'يتحسن',insufficient_data:'بيانات غير كافية',recovered:'متعافٍ',strong:'قوي'};function e(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}async function runTask(url,btn){btn.disabled=true;taskMsg.textContent='جارٍ تجهيز التدريب...';let r=await fetch(url,{method:'POST'}),x=await r.json().catch(()=>null);if(r.ok&&x.student_path){location.href=x.student_path;return}btn.disabled=false;taskMsg.textContent=typeof x?.detail==='string'?x.detail:(x?.detail?.message||'تعذر إنشاء التدريب')}async function load(){let ses=await fetch('/api/student/session',{cache:'no-store'}).then(r=>r.json()).catch(()=>({authenticated:false}));if(!ses.authenticated){location.href='/student';return}let r=await fetch('/api/student/study-queue',{cache:'no-store'}),x=await r.json().catch(()=>null);if(!r.ok){content.textContent=typeof x?.detail==='string'?x.detail:'تعذر تحميل الخطة';return}let s=x.progression_summary;content.outerHTML='<div class=box><div class=grid><div class=card><div class=muted>المهام</div><div class=big>'+x.summary.tasks+'</div></div><div class=card><div class=muted>عناصر تتراجع</div><div class="big bad">'+s.worsening_items+'</div></div><div class=card><div class=muted>تم التعافي</div><div class="big ok">'+s.recovered_items+'</div></div><div class=card><div class=muted>بمصدر شرح معتمد</div><div class=big>'+x.summary.study_tasks_with_approved_source+'</div></div></div></div><div class=box><h2>الأولوية الحالية</h2>'+ (x.queue.length?x.queue.map(t=>'<div class=task><b>'+t.priority+'. '+e(t.title)+'</b> <span class="'+(t.status==='worsening'||t.status==='persistent_weakness'?'bad':'warn')+'">('+e(statusLabels[t.status]||t.status)+')</span><div class=muted>'+e(t.reason)+' · الإتقان الحديث '+e(t.recent_mastery??'—')+'%'+(t.delta==null?'':' · التغير '+(t.delta>0?'+':'')+t.delta+'%')+'</div>'+(t.source?'<div class=muted>المصدر: '+e(t.source.filename)+' · الصفحات '+e(t.source.start_page)+'–'+e(t.source.end_page)+'</div>':'<div class=muted>لا توجد صفحات شرح معتمدة لهذا العنصر؛ لن يتم اختراع شرح بديل.</div>')+'<button onclick="runTask(\''+t.action_url+'\',this)">ابدأ التدريب</button></div>').join(''):'<p class=ok>لا توجد نقاط ضعف مؤكدة تحتاج خطة علاجية حاليًا.</p>')+'</div>'}load()</script></main></html>'''
...
ADMIN_PAGE = r'''<!doctype html><html lang="ar" dir="rtl"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ذكاء المذاكرة للطالب</title><style>
body{font-family:system-ui;background:#f5f7fb;color:#172033;margin:0}main{max-width:1100px;margin:auto;padding:16px}.box{background:#fff;border:1px solid #e4e7ec;border-radius:16px;padding:16px;margin:12px 0}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px}.card,.task{border:1px solid #e4e7ec;border-radius:12px;padding:12px}.task{margin:8px 0}.big{font-size:25px;font-weight:800}.bad{color:#b42318}.warn{color:#b54708}.ok{color:#067647}.muted{color:#667085}table{width:100%;border-collapse:collapse}td,th{padding:8px;border-bottom:1px solid #eee;text-align:right}a{color:#175cd3;text-decoration:none}</style><main>
<div class=box><a href="/admin/students">← إدارة الطلاب</a><h1>ذكاء المذاكرة للطالب</h1></div><div id=content class=box>جارٍ التحميل...</div>
<script>const id=Number(location.pathname.split('/')[3]);function e(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}async function load(){let [pr,qr]=await Promise.all([fetch('/api/admin/students/'+id+'/weakness-progression'),fetch('/api/admin/students/'+id+'/study-queue')]);if(!pr.ok||!qr.ok){content.textContent='تعذر تحميل بيانات الطالب';return}let p=await pr.json(),q=await qr.json(),s=p.summary;content.outerHTML='<div class=box><h2>'+e(p.student.name)+'</h2><div class=grid><div class=card><div class=muted>يتراجع</div><div class="big bad">'+s.worsening_items+'</div></div><div class=card><div class=muted>مفاهيم تحتاج متابعة</div><div class=big>'+s.concepts_needing_attention+'</div></div><div class=card><div class=muted>مهارات تحتاج متابعة</div><div class=big>'+s.skills_needing_attention+'</div></div><div class=card><div class=muted>تم التعافي</div><div class="big ok">'+s.recovered_items+'</div></div></div></div><div class=box><h2>قائمة المذاكرة الحالية</h2>'+q.queue.map(t=>'<div class=task><b>'+t.priority+'. '+e(t.title)+'</b> · <span class="'+(t.status==='worsening'||t.status==='persistent_weakness'?'bad':'warn')+'">'+e(t.status)+'</span><div class=muted>'+e(t.reason)+' · حديث '+e(t.recent_mastery??'—')+'%'+(t.delta==null?'':' · Δ '+(t.delta>0?'+':'')+t.delta+'%')+'</div>'+(t.source?'<div class=muted>المصدر: '+e(t.source.filename)+' ص '+e(t.source.start_page)+'–'+e(t.source.end_page)+'</div>':'')+'</div>').join('')+'</div>'+trend('المفاهيم',p.concepts)+trend('المهارات',p.skills)}function trend(title,a){return '<div class=box><h2>'+title+'</h2><table><tr><th>العنصر</th><th>آخر 5</th><th>الـ5 السابقة</th><th>التغير</th><th>الحالة</th></tr>'+a.map(x=>'<tr><td>'+e(x.item_title)+'</td><td>'+e(x.recent_mastery??'—')+'%</td><td>'+e(x.previous_mastery??'—')+'%</td><td>'+e(x.delta==null?'—':(x.delta>0?'+':'')+x.delta+'%')+'</td><td>'+e(x.status)+'</td></tr>').join('')+'</table></div>'}load()</script></main></html>'''


@app.get("/admin/students/{student_id}/study-intelligence", response_class=HTMLResponse, dependencies=[Depends(require_admin)])
def admin_study_intelligence_page(student_id: int):
    return ADMIN_PAGE
