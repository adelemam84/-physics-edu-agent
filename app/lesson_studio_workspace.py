...
async function exportFinal(){try{let r=await fetch(`/api/admin/lesson-studio/jobs/${encodeURIComponent(jobId())}/final-pdf`,{method:'POST'});if(r.status===401){location.href='/admin/login';return}if(!r.ok){let x=await r.json().catch(()=>null);throw new Error(JSON.stringify(x?.detail||x||r.status))}let blob=await r.blob();let u=URL.createObjectURL(blob);let a=document.createElement('a');a.href=u;a.download=`science-lesson-${jobId()}.pdf`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),1000)}catch(e){alert(e.message)}}
