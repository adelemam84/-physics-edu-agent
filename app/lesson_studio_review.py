...
def _refresh_transcript_and_structure(job_id: str) -> dict:
    """Rebuild transcript/structure and reject stale writes if sources change during organization."""
    _review_schema()
    ...
    transcript = _source_transcript(sources)
    ...
    with connect() as con:
        locked_job = con.execute(
            'SELECT id FROM science_lesson_jobs WHERE id=%s FOR UPDATE',
            (job_id,),
        ).fetchone()
        if not locked_job:
            raise HTTPException(404, 'Lesson studio job not found')
        current_sources = list(con.execute('''SELECT id,position,filename,extracted_text,requires_review
          FROM science_lesson_sources WHERE job_id=%s ORDER BY position FOR UPDATE''',
          (job_id,)).fetchall())
        current_transcript = _source_transcript(current_sources)
        current_pending = sum(1 for s in current_sources if s['requires_review'])
        if current_transcript != transcript or current_pending != pending:
            raise HTTPException(409, 'Lesson sources changed during transcript rebuild; reload and retry')
        if structured is None:
            con.execute('UPDATE science_lesson_jobs SET raw_transcript=%s WHERE id=%s', (transcript, job_id))
            invalidate_release_state(con, job_id, status='review_required')
        else:
            con.execute('UPDATE science_lesson_jobs SET raw_transcript=%s,structured_json=%s::jsonb WHERE id=%s',
...
def approve_lesson_source(job_id: str, source_id: int, approved_text: str = Form(...)):
    ...
    with connect() as con:
        ...
        if not job:
            raise HTTPException(404, 'Lesson studio job not found')
        row = con.execute('SELECT id FROM science_lesson_sources WHERE id=%s AND job_id=%s FOR UPDATE', (source_id, job_id)).fetchone()
        if not row:
...
def reopen_lesson_source(job_id: str, source_id: int):
    ...
    with connect() as con:
        ...
        invalidate_release_state(con, job_id, status='review_required')
    return {'reopened': True, 'source_id': source_id, 'previous_approval_invalidated': True, 'previous_pdf_invalidated': True}
