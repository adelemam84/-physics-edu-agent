...
</style><div class=layout><aside class=side><h2>منصة العلوم التعليمية</h2><a href="/admin/dashboard">الرئيسية</a><a href="/admin/workflow">مراجعة الأسئلة</a><a href="/admin/academic">الهيكل الأكاديمي</a><a href="/admin/bank">بنك الأسئلة</a><a href="/admin/quiz-builder">إنشاء اختبار</a><a href="/admin/students">الطلاب والاختبارات</a><a href="/admin/parents">أولياء الأمور وواتساب</a><a href="/admin/progress">متابعة التقدم</a><a href="/admin/readiness">جاهزية النظام</a><a href="/student">بوابة الطالب</a><a href="/practice">معاينة الطالب</a></aside><main class=main>
...
<script>
const H=()=>({});async function logout(){await fetch('/api/admin/logout',{method:'POST'});localStorage.removeItem('pk');location.href='/admin/login'}
function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}
async function load(){let s=await fetch('/api/admin/session').then(r=>r.json());if(!s.authenticated){location.href='/admin/login';return}msg.textContent='جلسة الإدارة آمنة ومفعلة';let r=await fetch('/api/admin/dashboard',{headers:H()});if(!r.ok){location.href='/admin/login';return}let x=await r.json(),s=x.summary||{};cards.innerHTML=[['الطلاب',s.students||0],['أولياء الأمور',s.guardians||0],['الاختبارات',s.quizzes||0],['الاختبارات المنشورة',s.published_quizzes||0],['المحاولات',s.attempts||0],['الأسئلة المعتمدة',s.approved_questions||0],['متوسط النتائج',(s.avg_score??0)+'%'],['رسائل في الانتظار',s.queued_notifications||0],['رسائل فاشلة',s.failed_notifications||0]].map(([a,b])=>`<div class=card><div class=muted>${a}</div><h2>${b}</h2></div>`).join('');
