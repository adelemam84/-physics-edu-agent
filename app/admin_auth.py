...
async function login(){msg.className='muted';msg.textContent='جارٍ التحقق...';let r=await fetch('/api/admin/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key:key.value})}),x=await r.json();if(!r.ok){msg.className='bad';msg.textContent=x.detail||'تعذر تسجيل الدخول';return}localStorage.removeItem('pk');msg.className='ok';msg.textContent='تم تسجيل الدخول';location.href='/admin/dashboard'}
