...
def knowledge_map(student_id:int):
    with connect() as con:
        ...
        return {"student":student,"concepts":concepts,"lessons":lessons}
...
<script>key.value=localStorage.pk||'';const id=Number(location.pathname.split('/').pop());const H=()=>({'X-Admin-Key':localStorage.pk||''});function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}function cls(v){return Number(v)<50?'low':Number(v)<75?'mid':'high'}async function load(){localStorage.pk=key.value;let r=await fetch('/api/admin/students/'+id+'/knowledge-map',{headers:H()});if(!r.ok){content.innerHTML='<div class=box>تعذر فتح الخريطة.</div>';return}let x=await r.json();content.innerHTML=`<div class=box><h1>${esc(x.student.name)}</h1><div class=muted>خريطة المعرفة مبنية فقط على نتائج الأسئلة المعتمدة المرتبطة بالمفاهيم.</div></div><div class=box><h2>المفاهيم</h2>${x.concepts.length?x.concepts.map(c=>`<div class="row ${cls(c.mastery)}"><div><b>${esc(c.title)}</b><div class=muted>${esc(c.subject_name||'')} · ${esc(c.unit_title||'')} · ${esc(c.lesson_title||'')} · ${c.attempts} إجابة</div><div class=bar><div class=fill style="width:${c.mastery||0}%"></div></div></div><b>${c.mastery||0}%</b></div>`).join(''):'لا توجد بيانات مفاهيم بعد'}</div><div class=box><h2>الدروس</h2>${x.lessons.map(c=>`<div class="row ${cls(c.mastery)}"><div><b>${esc(c.lesson_title)}</b><div class=muted>${esc(c.subject_name||'')} · ${esc(c.unit_title||'')} · ${c.attempts} إجابة</div><div class=bar><div class=fill style="width:${c.mastery||0}%"></div></div></div><b>${c.mastery||0}%</b></div>`).join('')}</div>`}</script></main></html>'''
