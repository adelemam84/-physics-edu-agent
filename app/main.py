    ...
    with connect() as con:
        ...
        if values.get('approved') is True:
            gate=con.execute("""SELECT q.id,q.document_id,coalesce(q.source_page,q.page) page_number,q.subject_id,q.grade_level_id,q.curriculum_version_id,q.term_id,q.unit_id,q.lesson_id,q.question_type,q.difficulty,q.accepted_answer,
              ...
              NOT EXISTS(SELECT 1 FROM question_concepts qc JOIN concepts c ON c.id=qc.concept_id
                WHERE qc.question_id=q.id AND c.lesson_id IS DISTINCT FROM q.lesson_id) concept_consistent,
              NOT EXISTS(SELECT 1 FROM curriculum_versions cv WHERE cv.id=q.curriculum_version_id AND cv.active=TRUE)
                OR EXISTS(SELECT 1 FROM document_page_reviews r
                  WHERE r.document_id=q.document_id AND r.page_number=coalesce(q.source_page,q.page)
                    AND r.page_role='question_candidate' AND r.review_status IN ('reviewing','done')) current_visual_page_reviewed
              FROM questions q WHERE q.id=%s""",(question_id,)).fetchone()
...
def pdf_url(document_id:int):
    with connect() as con:
        row=con.execute("""SELECT d.storage_url,f.object_key
    ...
    if row['object_key']:
        return {'url':presigned_get(row['object_key'],900),'expires_seconds':900,'source_type':'object_storage'}
    if row['storage_url']:
        return {'url':row['storage_url'],'expires_seconds':None,'source_type':'external_source'}
