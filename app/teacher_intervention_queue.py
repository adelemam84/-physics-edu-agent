    ...
    with connect() as con:
        rows = list(con.execute(
            """WITH completed AS (
                 ...
                 SELECT a.student_id,count(*) weak_lessons FROM (
               ...
               FROM student_perf p
               LEFT JOIN weak2 w2 ON w2.student_id=p.student_id
...
def intervention_queue(limit: int = 100, priority: str | None = None):
    ...
    if priority:
        ...
        data["items"] = [x for x in data["items"] if x["priority"] == priority][:min(max(limit,1),300)]
    else:
        data["items"] = data["items"][:min(max(limit,1),300)]
    return data
