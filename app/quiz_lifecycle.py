...
def lifecycle(status:str|None=None,limit:int=250):
    ...
    with connect() as con:
        rows=list(con.execute("""SELECT q.id,q.title,q.lifecycle_status,q.quality_score,q.published,q.created_at,
          q.ready_at,q.published_at,q.archived_at,q.max_attempts,q.retry_wait_minutes,q.score_policy,s.name_ar subject_name,g.name_ar grade_name,
...
function actions(q){let a='<button onclick="showAudit('+q.id+')">السجل</button> <a href="/admin/quiz-builder">فتح المنشئ</a> ';if(q.lifecycle_status==='draft'||q.lifecycle_status==='quality_review')a+='<button onclick="act('+q.id+',\'review\')">فحص الجودة</button> ';if(q.lifecycle_status==='ready')a+='<button onclick="act('+q.id+',\'publish\')">نشر</button> ';if(q.lifecycle_status==='published')a+='<button onclick="act('+q.id+',\'unpublish\')">إلغاء النشر</button> ';a+='<button onclick="policy('+q.id+','+q.max_attempts+','+q.retry_wait_minutes+',\''+q.score_policy+'\')">سياسة المحاولات</button> ';if(q.lifecycle_status!=='archived')a+='<button onclick="act('+q.id+',\'archive\')">أرشفة</button>';return a}
async function policy(id,max,wait,score){let m=prompt('عدد المحاولات المسموح بها (1-20)',max);if(m===null)return;let w=prompt('مدة الانتظار بين المحاولات بالدقائق (0 = بدون انتظار)',wait);if(w===null)return;let s=prompt('سياسة احتساب الدرجة: highest = أعلى درجة، latest = آخر درجة',score);if(s===null)return;let r=await fetch('/api/quizzes/'+id+'/attempt-policy',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({max_attempts:Number(m),retry_wait_minutes:Number(w),score_policy:s.trim()})}),x=await r.json();if(!r.ok){alert(x.detail||'تعذر حفظ السياسة');return}await load(current);await showAudit(id)}
