...
def _source_tutor_context(con, student_id: int):
    ...
    mapping = con.execute(
        """SELECT lsm.lesson_id,lsm.document_id,lsm.start_page,lsm.end_page,
                 d.filename,d.kind source_kind
          ...
          ORDER BY lsm.updated_at DESC,lsm.id DESC LIMIT 1""",
        (target["lesson_id"],),
    ).fetchone()
    if not mapping:
        return {
