...
.q{border:1px solid #e4e7ec;border-radius:12px;padding:12px;margin:8px 0}table{width:100%;border-collapse:collapse}td,th{padding:8px;border-bottom:1px solid #eee;text-align:right}a{color:#175cd3;text-decoration:none}
</style><main><div class=box><a href="/student">← بوابة الطالب</a></div><div id=content class=box>جارٍ تحميل التحليل...</div>
<script>
const id=Number(location.pathname.split('/').pop());function e(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}
function sec(v){v=Number(v||0);return v<60?v+' ث':Math.floor(v/60)+' د '+(v%60)+' ث'}
function rows(title,a){return '<div class=box><h2>'+title+'</h2><table><tr><th>البند</th><th>الإجابات</th><th>الإتقان</th></tr>'+a.map(x=>'<tr><td>'+e(x.label)+'</td><td>'+x.responses+'</td><td class="'+(Number(x.mastery)<60?'bad':Number(x.mastery)<80?'warn':'ok')+'">'+(x.mastery??'—')+'%</td></tr>').join('')+'</table></div>'}
async function load(){let r=await fetch('/api/student/attempts/'+id+'/diagnostic',{cache:'no-store'}),x=await r.json().catch(()=>null);if(!r.ok){content.textContent=typeof x?.detail==='string'?x.detail:'تعذر تحميل التحليل';return}
let a=x.attempt,s=x.summary;content.outerHTML='<div class=box><h1>'+e(a.quiz_title)+'</h1><div class=muted>'+e(a.student_name)+'</div><div class=grid><div class=card><div class=muted>النتيجة</div><div class=big>'+a.percentage+'%</div></div><div class=card><div class=muted>صحيح</div><div class="big ok">'+s.correct+'</div></div><div class=card><div class=muted>خطأ</div><div class="big bad">'+s.incorrect+'</div></div><div class=card><div class=muted>متوسط زمن السؤال</div><div class=big>'+sec(s.average_time_seconds)+'</div></div></div></div>'+rows('حسب الدرس',x.lessons)+rows('حسب المفهوم',x.concepts)+rows('حسب المهارة',x.skills)+'<div class=box><h2>تدريب نقاط الضعف</h2><p class=muted>ينشئ تدريبًا جديدًا من نقاط ضعف هذه المحاولة فقط، باستخدام أسئلة أخرى معتمدة من المصدر.</p><button onclick="weaknessPractice()">إنشاء تدريب نقاط الضعف</button><span id=weakMsg class=muted></span></div><div class=box><h2>الأخطاء التي تحتاج مراجعة</h2>'+(x.weak_questions.length?x.weak_questions.map(q=>'<div class=q><b>'+e(q.lesson_title||'غير مصنف')+'</b><p>'+e(q.text_verbatim)+'</p><div class=bad>إجابتك: '+e(q.answer_text||'بدون إجابة')+'</div><div class=ok>الإجابة المعتمدة: '+e(q.accepted_answer||'—')+'</div><div class=muted>المصدر: '+e(q.source_filename||'—')+' · صفحة '+e(q.source_page||'—')+' · الزمن '+sec(q.time_spent_seconds)+'</div>'+(q.solution_verbatim?'<div class=muted>الحل من المصدر: '+e(q.solution_verbatim)+'</div>':'')+'</div>').join(''):'<p class=ok>لا توجد أخطاء في هذه المحاولة.</p>')+'</div><div class=box><h2>الأسئلة التي استغرقت وقتًا أطول</h2>'+x.slow_questions.map(q=>'<div class=q><b>'+sec(q.time_spent_seconds)+'</b> · '+e(q.lesson_title||'غير مصنف')+'<div>'+e(q.text_verbatim)+'</div></div>').join('')+'</div>'}
async function weaknessPractice(){weakMsg.textContent=' جارٍ إنشاء التدريب...';let r=await fetch('/api/student/attempts/'+id+'/weakness-practice/create?count=10',{method:'POST'}),x=await r.json().catch(()=>null);if(!r.ok){weakMsg.textContent=' '+(typeof x?.detail==='string'?x.detail:(x?.detail?.message||'تعذر إنشاء التدريب'));return}location.href=x.student_path}
load()
</script></main></html>'''
