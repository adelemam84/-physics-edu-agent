...
if _API_OLD not in workspace.WORKSPACE:
    raise RuntimeError(
        'Lesson Studio Workspace API helper marker is missing; refusing startup because mutation preconditions would not be sent'
    )
workspace.WORKSPACE = workspace.WORKSPACE.replace(_API_OLD, _API_NEW, 1)

if _APPROVE_SOURCE_OLD not in workspace.WORKSPACE:
    raise RuntimeError(
        'Lesson Studio Workspace source-approval marker is missing; refusing startup because source revision preconditions would not be sent'
    )
