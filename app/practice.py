...
def practice_questions(limit: int = 100):
    with connect() as con:
        rows = con.execute(
            '''
            SELECT q.id, q.text_verbatim, q.question_type, q.accepted_answer,
...
def practice_asset(question_id: int):
    ...
    return Response(get_bytes(row['object_key']), media_type='image/jpeg', headers={'Cache-Control':'public, max-age=300'})


PRACTICE = r'''<!doctype html><html lang="ar" dir="rtl"><meta name="viewport" content="width=device-width,initial-scale=1"><title>تدريب الفيزياء</title><style>body{font-family:system-ui;background:#f5f7fb;margin:0;color:#172033}main{max-width:900px;margin:auto;padding:18px}.card{background:white;border-radius:16px;padding:18px;margin:14px 0;box-shadow:0 3px 14px #0000000a}.asset{width:100%;max-height:70vh;object-fit:contain;border-radius:10px;background:#fafafa}.meta{color:#667085;font-size:13px}.text{white-space:pre-wrap;line-height:1.8}.empty{text-align:center;padding:50px 10px;color:#667085}</style><main><h1>تدريب الفيزياء</h1><p>يظهر هنا فقط ما تم اعتماده من بنك الأسئلة.</p><div id=items></div></main><script>fetch('/api/practice/questions').then(r=>r.json()).then(qs=>{items.innerHTML=qs.length?qs.map(q=>`<article class=card><div class=meta>#${q.id} · ${q.source_filename} · صفحة ${q.source_page}</div>${q.has_asset?`<img class=asset src="/api/practice/questions/${q.id}/asset">`:''}<div class=text>${esc(q.text_verbatim)}</div></article>`).join(''):'<div class=empty>لا توجد أسئلة معتمدة للطلاب حتى الآن.</div>'});function esc(s){return String(s).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}</script></html>'''
